...
def demo(opt):
    ...
    predictions = []
    count = 0
    if not os.path.exists('Visu/visu_'+opt.save_in):
            os.makedirs('Visu/visu_'+opt.save_in)
    ...
    for step in range(1000):
        ...
        sents, det_idx, det_word = utils.decode_sequence_det(dataset_val.itow, dataset_val.itod, dataset_val.ltow, dataset_val.itoc, dataset_val.wtod, seq, bn_seq, fg_seq, opt.vocab_size, opt)
        ...
        fig.savefig('Visu/visu_'+opt.save_in+'/%d.jpg' %(img_id[0]), bbox_inches='tight', pad_inches=0, dpi=250)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_in', type=str, default='', help='')
    ...
    parser.add_argument('--image_path', type=str, default='data/coco/images/',help='path to the h5file containing the image data')
    parser.add_argument('--cbs', type=bool, default=False,help='whether use constraint beam search.')
    parser.add_argument('--cbs_tag_size', type=int, default=3,help='whether use constraint beam search.')
    parser.add_argument('--cbs_mode', type=str, default='all',help='which cbs mode to use in the decoding stage. cbs_mode: all|unique|novel')
    parser.add_argument('--det_oracle', type=bool, default=False,help='whether use oracle bounding box.')
    parser.add_argument('--cnn_backend', type=str, default='res101',help='res101 or vgg16')
    ...
    json.dump(predictions, open('preds_'+opt.save_in+'.json', 'w'))
    
    #f = open('result.txt', 'wb')
    #for i,j in enumerate(predictions)
    #    entry = {'image_id': img_id[k].item(), 'caption': sent}
    #    f.write("%d : %s \n" % (predictions['image_id'], predictions['caption']))    
    #f.close()
